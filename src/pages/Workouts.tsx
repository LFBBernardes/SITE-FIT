    ...
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {bodyParts.map((part) => (
          <Link 
            key={part.id} 
            to={`/${lang}/workouts/${getLocalizedSlug('workouts', lang as Language, part.id)}`}
      ...
      <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-zinc-900 text-white p-8 rounded-3xl">
          ...
          <Link to={`/${lang}/workouts/${getLocalizedSlug('workouts', lang as Language, 'home-workouts')}`} className="text-emerald-400 font-bold flex items-center gap-2">{lang === 'en' ? 'Explore' : lang === 'es' ? 'Explorar' : 'Explorar'} <ChevronRight className="h-4 w-4" /></Link>
        </div>
        <div className="bg-zinc-100 p-8 rounded-3xl">
          ...
          <p className="text-zinc-500 mb-6 text-sm">{lang === 'en' ? 'Start your journey with structured, safe, and effective introductory routines.' : lang === 'es' ? 'Comienza tu viaje con rutinas introductorias estructuradas, seguras y efectivas.' : 'Comece sua jornada com rotinas introductorias estruturadas, seguras e eficazes.'}</p>
          <Link to={`/${lang}/workouts/${getLocalizedSlug('workouts', lang as Language, 'beginner')}`} className="text-black font-bold flex items-center gap-2">{lang === 'en' ? 'Explore' : lang === 'es' ? 'Explorar' : 'Explorar'} <ChevronRight className="h-4 w-4" /></Link>
        </div>
        <div className="bg-zinc-100 p-8 rounded-3xl">
          ...
          <Link to={`/${lang}/workouts/${getLocalizedSlug('workouts', lang as Language, 'advanced')}`} className="text-black font-bold flex items-center gap-2">{lang === 'en' ? 'Explore' : lang === 'es' ? 'Explorar' : 'Explorar'} <ChevronRight className="h-4 w-4" /></Link>
        </div>
      </div>
    </div>
  );
